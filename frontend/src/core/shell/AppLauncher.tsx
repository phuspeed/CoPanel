...
export default function AppLauncher({ open, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[210] bg-slate-950/70 backdrop-blur-md flex flex-col" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="flex-1 flex flex-col items-center pt-16 px-6">
        <div className="w-full max-w-3xl">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-6">
            ...
            {filtered.length === 0 && (
              <p className="col-span-full text-center text-sm text-slate-300">No apps match your search.</p>
            )}
          </div>
        </div>
      </div>
