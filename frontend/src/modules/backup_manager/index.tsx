...
export default function BackupAndSyncDashboard() {
  ...
  const computedRedirectUri = isIp
    ? `http://${window.location.hostname}.nip.io:${window.location.port || '8686'}/backup-manager`
    : `${window.location.origin}/backup-manager`;
  // When returning from Google OAuth, use the URI that was saved before redirect
  const redirectUri = localStorage.getItem('copanel_oauth_redirect_uri') || computedRedirectUri;
  ...
  useEffect(() => {
    ...
    // Catch query code parameter from Google OAuth redirect
    const params = new URLSearchParams(window.location.search);
    const code = params.get('code');
    if (code) {
      // Read credentials saved to localStorage BEFORE the OAuth redirect
      // because config state is still empty (fetchConfigAndCrons not done yet)
      const savedClientId = localStorage.getItem('copanel_oauth_client_id') || '';
      const savedClientSecret = localStorage.getItem('copanel_oauth_client_secret') || '';
      const savedRedirectUri = localStorage.getItem('copanel_oauth_redirect_uri') || computedRedirectUri;
      handleExchangeCode(code, savedClientId, savedClientSecret, savedRedirectUri);
    }
  }, []);
  ...
  const handleExchangeCode = async (
    ...
    const useClientId = clientId || config.google_drive_client_id;
    const useClientSecret = clientSecret || config.google_drive_client_secret;
    const useRedirectUri = overrideRedirectUri || redirectUri;

    if (!useClientId || !useClientSecret) {
      setMsg({
        text: language === 'vi'
          ? '⚠ Không tìm thấy Client ID / Secret. Hãy nhập và lưu cấu hình trước khi xác thực.'
          : '⚠ Client ID or Secret not found. Please save your config before authenticating.',
        isError: true
      });
      return;
    }
    ...
    try {
      ...
      // Clean up localStorage after exchange attempt
      localStorage.removeItem('copanel_oauth_client_id');
      localStorage.removeItem('copanel_oauth_client_secret');
      localStorage.removeItem('copanel_oauth_redirect_uri');

      if (res.ok) {
        setMsg({ text: language === 'vi' ? '✓ Xác thực Google Drive thành công! Token đã được lưu.' : '✓ Google Drive authorized successfully! Token saved.', isError: false });
        fetchConfigAndCrons();
      } else {
        const data = await res.json();
        setMsg({ text: data.detail || (language === 'vi' ? 'Không thể trao đổi mã token với Google.' : 'Failed to exchange token with Google.'), isError: true });
      }
    } catch {
  ...
  const handleDirectOAuth = () => {
    ...
    if (!config.google_drive_client_secret) {
      setMsg({ text: language === 'vi' ? 'Vui lòng nhập Google Client Secret trước khi xác thực.' : 'Please enter the Google Client Secret before authorization.', isError: true });
      return;
    }
    // Save credentials to localStorage BEFORE leaving this page
    // so they can be used when Google redirects back (config state will be empty then)
    localStorage.setItem('copanel_oauth_client_id', config.google_drive_client_id);
    localStorage.setItem('copanel_oauth_client_secret', config.google_drive_client_secret);
    localStorage.setItem('copanel_oauth_redirect_uri', computedRedirectUri);

    const oauthUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${encodeURIComponent(config.google_drive_client_id)}&redirect_uri=${encodeURIComponent(computedRedirectUri)}&response_type=code&scope=https://www.googleapis.com/auth/drive&access_type=offline&prompt=consent`;
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 md:space-y-8 select-none">
      ...
      <div className={`relative overflow-hidden border p-6 md:p-8 rounded-2xl backdrop-blur-md shadow-xl flex flex-col md:flex-row items-center justify-between gap-6 transition-all duration-300 ${isDark
        ...
        <div className={`flex flex-col items-center md:items-end gap-1 text-center md:text-right p-4 rounded-xl border backdrop-blur-sm self-stretch md:self-auto min-w-[180px] transition duration-200 ${isDark ? 'bg-slate-900/50 border-slate-800/80' : 'bg-white border-slate-200'
          }`}>
      ...
      {msg && (
        <div className={`p-4 border rounded-xl text-xs flex items-center gap-2 max-w-xl animate-fade-in ${msg.isError
      ...
      <div className="flex border-b mb-6 select-none overflow-x-auto gap-1">
        <button
          ...
          className={`flex items-center gap-2 px-6 py-3 text-xs font-bold transition duration-200 border-b-2 shrink-0 ${activeTab === 'rclone'
        ...
        <button
          ...
          className={`flex items-center gap-2 px-6 py-3 text-xs font-bold transition duration-200 border-b-2 shrink-0 ${activeTab === 'scheduled'
              ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400 bg-indigo-50/20'
              : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'
            }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-6 py-3 text-xs font-bold transition duration-200 border-b-2 shrink-0 ${activeTab === 'realtime'
              ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400 bg-indigo-50/20'
              : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'
            }`}
        >
      ...
      <div>
        {activeTab === 'rclone' && (
          <form onSubmit={handleSaveConfig} className={`border p-6 md:p-8 rounded-2xl backdrop-blur-sm space-y-5 transition-all duration-300 h-fit max-w-xl mx-auto ${isDark ? 'bg-slate-900/50 border-slate-800/80' : 'bg-white border-slate-200'
            }`}>
            ...
            <div className="space-y-4">
              <div className={`border p-4 rounded-xl space-y-3 ${isDark ? 'bg-indigo-950/20 border-indigo-900/30' : 'bg-indigo-50/40 border-indigo-100'}`}>
                ...
                <code className={`p-2 font-mono text-xs select-all block rounded-lg border text-center font-bold tracking-wide break-all ${isDark ? 'bg-slate-950/80 border-slate-800 text-indigo-400' : 'bg-white border-slate-200 text-indigo-600'
                  }`}>
                  {redirectUri}
                </code>
                ...
                <div className={`border-t pt-2 mt-2 space-y-1 ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
                  ...
                  <code className={`p-1.5 font-mono text-[10px] select-all block rounded border break-all ${isDark ? 'bg-slate-950 border-slate-800 text-green-400' : 'bg-white border-slate-200 text-green-600'
                    }`}>
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  className={`w-full border rounded-xl px-4 py-2.5 text-xs focus:outline-none focus:border-indigo-500 transition-all ${isDark ? 'bg-slate-950 border-slate-800 text-slate-100' : 'bg-slate-50/50 border-slate-200 text-slate-800'
                    }`}
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  className={`w-full border rounded-xl px-4 py-2.5 text-xs focus:outline-none focus:border-indigo-500 font-mono transition-all ${isDark ? 'bg-slate-950 border-slate-800 text-slate-100' : 'bg-slate-50/50 border-slate-200 text-slate-800'
                    }`}
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  rows={2}
                  className={`w-full border rounded-xl px-4 py-2.5 text-xs focus:outline-none focus:border-indigo-500 transition-all font-mono ${isDark ? 'bg-slate-950 border-slate-800 text-slate-100' : 'bg-slate-50/50 border-slate-200 text-slate-800'
                    }`}
                />
              </div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className={`px-4 py-3 border font-bold text-xs rounded-xl flex items-center gap-2 transition duration-200 shrink-0 ${isDark ? 'bg-slate-800 border-slate-700 text-slate-300 hover:text-indigo-400' : 'bg-slate-50 border-slate-200 text-slate-700 hover:text-indigo-600'
                    }`}
                >
        ...
        {activeTab === 'scheduled' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
            <div className={`border p-6 md:p-8 rounded-2xl backdrop-blur-sm space-y-4 transition duration-300 h-fit ${isDark ? 'bg-slate-900/50 border-slate-800/80' : 'bg-white border-slate-200'
              }`}>
              <h3 className={`text-sm font-bold uppercase tracking-wider flex items-center gap-2 ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>
              ...
              <div className="space-y-4">
                <div className="space-y-1">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className={`flex-1 border rounded-xl px-4 py-2.5 text-xs focus:outline-none focus:border-indigo-500 transition-all ${isDark ? 'bg-slate-950 border-slate-800 text-slate-100' : 'bg-slate-50/50 border-slate-200 text-slate-800'
                        }`}
                    />
                    <button
                      type="button"
                      onClick={() => openDirectoryPicker('source_dir', taskSourceDir)}
                      className={`px-4 py-2 text-xs border rounded-xl flex items-center gap-1 hover:border-indigo-500 font-medium transition duration-200 ${isDark ? 'bg-slate-800 border-slate-700 text-slate-300 hover:text-indigo-400' : 'bg-slate-100 border-slate-200 text-slate-700 hover:text-indigo-600'
                        }`}
                    >
                  ...
                  <div className="flex items-center gap-4 flex-wrap">
                    <div className="flex items-center gap-2">
                      ...
                      <select
                        ...
                        className={`border rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-indigo-500 font-mono transition-all ${isDark ? 'bg-slate-950 border-slate-800 text-slate-100' : 'bg-slate-50/50 border-slate-200 text-slate-800'
                          }`}
                      >
                ...
                <div className="flex gap-2 flex-wrap pt-2">
                  ...
                  <button
                    ...
                    className={`px-5 py-3 border font-bold text-xs rounded-xl flex items-center gap-2 transition duration-200 ${isDark ? 'bg-slate-800 border-slate-700 text-slate-300 hover:text-indigo-400' : 'bg-slate-50 border-slate-200 text-slate-700 hover:text-indigo-600'
                      }`}
                  >
                    <Icons.Save className="w-4 h-4" />
              ...
              {config.backup_tasks && config.backup_tasks.length > 0 ? (
                <div className="space-y-3">
                  {config.backup_tasks.map((t: any, idx: number) => (
                    <div
                      key={idx}
                      className={`p-4 rounded-xl border flex flex-wrap items-center justify-between gap-4 transition duration-200 ${isDark ? 'bg-slate-950 border-slate-800/80 hover:border-slate-700' : 'bg-slate-50/50 border-slate-200/80 hover:border-slate-300'
                        }`}
                    >
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className={`p-2 border rounded-xl transition ${isDark ? 'bg-slate-800 hover:bg-slate-700 border-slate-700 text-indigo-400 hover:text-indigo-300' : 'bg-white hover:bg-indigo-50 border-slate-200 text-indigo-600 hover:text-indigo-700'
                            }`}
                        ...
                        <button
                          ...
                          className={`p-2 border rounded-xl transition ${isDark ? 'bg-slate-800 hover:bg-slate-700 border-slate-700 text-red-400 hover:text-red-300' : 'bg-white hover:bg-red-50 border-slate-200 text-red-600 hover:text-red-700'
                            }`}
        ...
        {activeTab === 'realtime' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
              ...
              <div className="space-y-4">
                <div className="space-y-1">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className={`flex-1 border rounded-xl px-3 py-2.5 text-xs focus:outline-none focus:border-indigo-500 transition-all font-mono ${isDark ? 'bg-slate-900/60 border-slate-800/60 text-slate-100' : 'bg-white border-slate-200 text-slate-800'
                        }`}
                    />
                    <button
                      type="button"
                      onClick={() => openDirectoryPicker('local_path', localPath)}
                      className={`px-3 py-2 text-xs border rounded-xl flex items-center gap-1 hover:border-indigo-500 font-medium transition duration-200 ${isDark ? 'bg-slate-800 border-slate-700 text-slate-300 hover:text-indigo-400' : 'bg-slate-100 border-slate-200 text-slate-700 hover:text-indigo-600'
                        }`}
                    >
                      <Icons.Folder className="w-4 h-4" />
                    </button>
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className={`w-full border rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-indigo-500 transition-all font-mono ${isDark ? 'bg-slate-900/60 border-slate-800/60 text-slate-100' : 'bg-white border-slate-200 text-slate-800'
                      }`}
                  />
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    className={`w-full border rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-indigo-500 transition-all ${isDark ? 'bg-slate-900/60 border-slate-800/60 text-slate-100' : 'bg-white border-slate-200 text-slate-800'
                      }`}
                  >
            ...
            <div className={`border p-6 md:p-8 rounded-2xl backdrop-blur-sm space-y-4 transition duration-300 h-fit flex flex-col justify-between ${isDark ? 'bg-slate-900/50 border-slate-800/80' : 'bg-white border-slate-200'
              }`}>
              <h3 className={`text-sm font-bold uppercase tracking-wider flex items-center gap-2 ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>
                <Icons.List className={`w-5 h-5 ${isDark ? 'text-indigo-400' : 'text-indigo-600'}`} />
                  ...
                  {config.sync_rules.map((r: any, idx: number) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className={`p-2 border rounded-xl transition shrink-0 ${isDark ? 'bg-slate-800 hover:bg-red-950/40 border-slate-700 hover:border-red-900/40 text-red-400' : 'bg-white hover:bg-red-50 border-slate-200 hover:border-red-200 text-red-600'
                          }`}
                      >
                        <Icons.Trash2 className="w-4 h-4" />
                      </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
        ...
        <div className={`border p-6 md:p-8 rounded-2xl backdrop-blur-sm space-y-4 flex flex-col transition-all duration-300 ${isDark ? 'bg-slate-900/50 border-slate-800/80' : 'bg-white border-slate-200'
          }`}>
          <h3 className={`text-sm font-bold uppercase tracking-wider flex items-center gap-2 ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>
          ...
          {crons && crons.length > 0 ? (
            <div className="space-y-2 overflow-y-auto max-h-[200px] pr-1">
              {crons.map((cron, idx) => (
                <div key={idx} className={`p-3 border rounded-xl text-xs flex flex-col gap-1 ${isDark ? 'bg-slate-950/60 border-slate-800/60' : 'bg-slate-50 border-slate-100'
                  }`}>
                  <div className="flex items-center justify-between">
                    <span className={`font-mono font-bold px-2 py-0.5 border rounded-lg text-indigo-500 dark:text-indigo-300 ${isDark ? 'bg-indigo-500/10 border-indigo-500/20' : 'bg-indigo-50 border-indigo-100'
                      }`}>
                      {cron.expression}
                    </span>
                    <button
                      onClick={handleDeleteCrons}
                      className={`text-[10px] px-2.5 py-1 rounded-lg font-bold border transition duration-150 ${isDark ? 'bg-red-950/40 hover:bg-red-900/60 border-red-800/60 text-red-400' : 'bg-red-50 hover:bg-red-100 border-red-200 text-red-600'
                        }`}
                    >
                      {tr.clearCrons}
                    </button>
            ...
            <div className={`text-xs border p-4 rounded-xl text-center md:text-left ${isDark ? 'text-slate-500 border-slate-800/40 bg-slate-950/20' : 'text-slate-400 border-slate-100 bg-slate-50/50'
              }`}>
              {tr.noCrons}
            </div>
          )}
          ...
          {backups && backups.length > 0 ? (
            <div className="space-y-2 overflow-y-auto max-h-[200px] pr-1">
              {backups.map((backup, idx) => (
                <div key={idx} className={`p-3 border rounded-xl text-xs flex items-center justify-between gap-3 animate-fade-in ${isDark ? 'bg-slate-950/60 border-slate-800/60' : 'bg-slate-50 border-slate-100'
                  }`}>
                  ...
                  <button
                    ...
                    className={`p-2 rounded-xl border transition shrink-0 ${isDark ? 'bg-slate-900 hover:bg-red-950/60 border-slate-800 hover:border-red-900/60 text-red-400' : 'bg-white hover:bg-red-50 border-slate-200 hover:border-red-200 text-red-600'
                      }`}
                    title="Delete"
                  >
                    <Icons.Trash2 className="w-4 h-4" />
      ...
      {dirPickerOpen && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
          <div className={`w-full max-w-lg p-6 rounded-2xl border shadow-2xl flex flex-col max-h-[85vh] transition duration-300 ${isDark ? 'bg-slate-900 border-slate-800 text-slate-100' : 'bg-white border-slate-200 text-slate-800'
            }`}>
            ...
            <div className="flex items-center gap-2 mb-3">
              <button
                ...
                className={`p-2 border rounded-xl transition ${isDark ? 'bg-slate-800 hover:bg-slate-700 border-slate-700 text-slate-300 disabled:opacity-40' : 'bg-slate-50 hover:bg-slate-100 border-slate-200 text-slate-600 disabled:opacity-40'
                  }`}
              ...
              <input
                ...
                className={`flex-1 border rounded-xl px-3 py-2 text-xs font-mono focus:outline-none focus:border-indigo-500 transition duration-200 ${isDark ? 'bg-slate-950 border-slate-800 text-indigo-400' : 'bg-slate-50 border-slate-200 text-indigo-600'
                  }`}
              />
              <button
                type="button"
                onClick={() => fetchDirs(currentPickPath)}
                className={`p-2 border rounded-xl transition ${isDark ? 'bg-slate-800 hover:bg-slate-700 border-slate-700 text-slate-300' : 'bg-slate-50 hover:bg-slate-100 border-slate-200 text-slate-600'
                  }`}
              >
            ...
            <div className={`flex-1 overflow-y-auto border rounded-xl p-2 min-h-[250px] space-y-1 mb-4 select-none ${isDark ? 'bg-slate-950/40 border-slate-800/80' : 'bg-slate-50/50 border-slate-200'
              }`}>
              {dirList.length > 0 ? (
                dirList.map((item: any, idx: number) => (
                  <div
                    ...
                    className={`p-2.5 rounded-xl flex items-center justify-between text-xs cursor-pointer transition ${isDark ? 'hover:bg-indigo-950/40 border border-transparent hover:border-indigo-800/60' : 'hover:bg-indigo-50 border border-transparent hover:border-indigo-200'
                      }`}
                  >
                    <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg border text-[10px] font-bold uppercase tracking-wider transition ${isDark ? 'bg-slate-900 border-slate-800 hover:bg-indigo-600 hover:border-indigo-600 text-indigo-400 hover:text-white' : 'bg-white border-slate-200 hover:bg-indigo-600 hover:border-indigo-600 text-indigo-600 hover:text-white'
                        }`}
                    >
            ...
            <div className="flex items-center justify-end gap-2 border-t pt-3">
              <button
                type="button"
                onClick={() => setDirPickerOpen(false)}
                className={`px-4 py-2 rounded-xl border text-xs font-bold transition ${isDark ? 'bg-slate-800 border-slate-700 text-slate-300 hover:bg-slate-700' : 'bg-slate-100 border-slate-200 text-slate-700 hover:bg-slate-200'
                  }`}
              >
