...
|--------|----------|-------------|
| **Classic (Web UI)** | Mặc định `install.sh`, hoặc `--classic` | Sidebar + module full-page |
| **Desktop** | `install.sh --desktop`, hoặc bật toggle | Desktop Deepin, dock, cửa sổ nổi |
...
**`scripts/install.sh`** — cài thống nhất trên nhánh `main`. Chọn giao diện khi cài.

**Tương tác** (clone local / SSH):

```bash
sudo bash scripts/install.sh
# Hỏi: 1) Web UI (classic)  2) Desktop UI
```
